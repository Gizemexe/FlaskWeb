...
mock = Blueprint('mock', __name__, url_prefix='/api/v1/mock')

# Simulate tokenization by generating a UUID as a token
@mock.route('/tokenize', methods=['POST'])
def tokenize():
    data = request.json
...
@mock.route('/detokenize', methods=['POST'])
def detokenize():
    data = request.json
...
@mock.route('/process_payment', methods=['POST'])
def process_payment():
    data = request.json
...
@mock.route('/finalize_order', methods=['POST'])
def finalize_order():
    data = request.json
